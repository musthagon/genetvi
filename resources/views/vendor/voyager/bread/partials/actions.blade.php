@if($data)
    ...
    @can ($action->getPolicy(), $data)
        ...
        @endphp
        <a href="{{ $action->getRoute($dataType->name) }}" title="{{ $action_name }} " {!! $action->convertAttributesToHtml() !!}>
            <i class="{{ $action->getIcon() }}"></i> <span class="hidden-xs hidden-sm">{{ $action_name }}</span>
        </a>
    @endcan
